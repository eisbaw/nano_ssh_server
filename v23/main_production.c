...
static void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    while (!requests_done) {
        ...
        if (want_reply) {
            uint8_t reply_msg[8];  /* Reduced from 16: only need 5 bytes (msg+channel_id) */
            size_t reply_len = 0;
    ...
    uint8_t eof_msg[8];  /* Reduced from 16: only need 5 bytes */
    size_t eof_len = 0;
    ...
    uint8_t close_msg[8];  /* Reduced from 16: only need 5 bytes */
    size_t close_len = 0;
