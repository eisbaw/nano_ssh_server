 ...
 * Custom DH implementation - converted to macros for inline expansion
 */
...
/* Write SSH string as inline function */
...
/* Calculate padding as inline macro */
static inline uint8_t calculate_padding(size_t payload_len, size_t block_size) {
    ...
    return (padding < MIN_PADDING) ? padding + block_size : padding;
}
...
void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    if (strcmp(method, "password") == 0) {
        /* Skip change password flag (not used in minimal implementation) */
        ptr += 1;
    ...
    /* Skip maximum_packet_size (not used) */
    ch_ptr += 4;
