/*
 * Nano SSH Server - v14-dunkels3
 * World's smallest SSH server for microcontrollers
 *
 * Dunkels Optimization 3: Packed Structures
 *
 * This version applies Adam Dunkels' packed structure optimization:
 * - __attribute__((packed)) to eliminate padding
 * - Struct member reordering (largest to smallest)
 * - Bitfields for boolean flags
 * - Minimal binary size through structure optimization
 */
