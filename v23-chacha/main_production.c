...
#include "chacha20poly1305_minimal.h"
...
#define BLOCK_SIZE_CHACHA 8        /* Stream cipher: pad to multiple of 8 */
...
#define ENCRYPTION_ALGORITHM    "chacha20-poly1305@openssh.com"  /* AEAD, OpenSSH variant */
#define MAC_ALGORITHM           ""                /* empty: AEAD provides integrity */
#define COMPRESSION_ALGORITHM   "none"
#define LANGUAGE                ""

/* chacha20-poly1305@openssh.com (AEAD):
 *  - 64-byte key K = K_2 || K_1
 *  - K_1 encrypts 4-byte packet length (ChaCha20 counter 0)
 *  - K_2 encrypts payload (ChaCha20 counter 1)
 *  - Poly1305 key = ChaCha20(K_2, nonce, counter 0)[0..31]
 *  - Poly1305 MAC over full ciphertext (enc_len || enc_payload)
 *  - nonce = big-endian uint64 sequence number
 */
...
typedef struct {
    uint8_t key[64];   /* chacha20-poly1305 64-byte key (K_2 || K_1) */
    uint32_t seq_num;
    int active;
} crypto_state_t;
...
int send_packet(int fd, const uint8_t *payload, size_t payload_len) {
    ...
    uint8_t mac[16];  /* Poly1305 produces 16-byte tag */
    uint8_t padding_len;
    uint32_t packet_len;
    size_t total_len;
    size_t block_size;
    ...
    /* Padding block size: 8 (stream cipher) in both states.
     * For chacha20-poly1305 the 4-byte length field is excluded from the
     * modulus, but since 4 is not a multiple of 8 and the standard formula
     * uses (4 + 1 + payload + pad), we follow OpenSSH and pad the encrypted
     * portion (1 + payload + pad) to a multiple of 8. calculate_padding()
     * uses (5 + payload), which already yields a multiple-of-8 total; for the
     * AEAD case the resulting (1+payload+pad) is also a multiple of 8 because
     * 4 mod 8 != 0 would shift it -- so compute padding over the encrypted
     * portion explicitly. */
    block_size = BLOCK_SIZE_CHACHA;

    if (encrypt_state_s2c.active) {
        /* pad so that (1 + payload_len + padding_len) % 8 == 0 */
        size_t enc_portion = 1 + payload_len;
        padding_len = block_size - (enc_portion % block_size);
        if (padding_len < MIN_PADDING) padding_len += block_size;
    } else {
        padding_len = calculate_padding(payload_len, BLOCK_SIZE_UNENCRYPTED);
    }
    ...
    /* If encryption is active, seal with chacha20-poly1305 and append tag */
    if (encrypt_state_s2c.active) {
        ssh_chacha_poly_seal(packet, total_len, encrypt_state_s2c.key,
                             (uint64_t)encrypt_state_s2c.seq_num, mac);

        /* Send encrypted packet (enc_len || enc_payload) */
        if (send_data(fd, packet, total_len) < 0) {
            return -1;
        }

        /* Send Poly1305 tag (16 bytes) */
...
ssize_t recv_packet(int fd, uint8_t *payload, size_t payload_max) {
    ...
    if (encrypt_state_c2s.active) {
        /* ===== ENCRYPTED MODE (chacha20-poly1305@openssh.com) ===== */

        /* Read the 4-byte encrypted length field */
        uint8_t enc_len[4];
        n = recv_data(fd, enc_len, 4);
        if (n <= 0) {
        ...
        /* Decrypt only the length (K_1, counter 0) WITHOUT consuming state */
        packet_len = ssh_chacha_decrypt_length(enc_len, encrypt_state_c2s.key,
                                               (uint64_t)encrypt_state_c2s.seq_num);
        ...
        /* Full ciphertext = enc_len(4) || enc_payload(packet_len) */
        total_packet_len = 4 + packet_len;

        /* Allocate buffer for full ciphertext (length field + payload) */
        packet_buf = malloc(total_packet_len);
        if (!packet_buf) {
            return -1;
        }

        /* Store the still-encrypted length field; MAC covers it */
        memcpy(packet_buf, enc_len, 4);

        /* Read the encrypted payload (packet_len bytes) */
        ...
        /* Verify MAC over full ciphertext and decrypt in place */
        if (ssh_chacha_poly_open(packet_buf, total_packet_len, encrypt_state_c2s.key,
                                 (uint64_t)encrypt_state_c2s.seq_num, mac) != 0) {
            free(packet_buf);
            return -1;
        }
...
void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    uint8_t key_c2s[64];     /* chacha20-poly1305 key client->server (K_2||K_1) */
    uint8_t key_s2c[64];     /* chacha20-poly1305 key server->client (K_2||K_1) */

    /* For chacha20-poly1305@openssh.com (AEAD):
     * - No IV (nonce derived from sequence number)
     * - No separate MAC key (Poly1305 key derived per-packet)
     * - 64-byte encryption key per direction: 'C' = c2s, 'D' = s2c
     */
    derive_key(key_c2s, 64, shared_secret, 32, exchange_hash, 'C', session_id);
    derive_key(key_s2c, 64, shared_secret, 32, exchange_hash, 'D', session_id);
    ...
    memcpy(encrypt_state_s2c.key, key_s2c, 64);
    ...
    memcpy(encrypt_state_c2s.key, key_c2s, 64);
