/*
 * Nano SSH Server - v14-crypto2
 * Crypto optimization: Minimal AES-128-CTR implementation
...
ssize_t recv_packet(int fd, uint8_t *payload, size_t payload_max) {
    ...
    if (encrypt_state_c2s.active) {
        ...
        aes128_ctr_xor(&encrypt_state_c2s.ctx, decrypted_block, 16);
        ...
        if (remaining > 0) {
            ...
            aes128_ctr_xor(&encrypt_state_c2s.ctx, packet_buf + 16, remaining);
...
void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    aes128_ctr_init(&encrypt_state_s2c.ctx, encrypt_state_s2c.enc_key, encrypt_state_s2c.iv);
    ...
    /* Initialize AES-128-CTR context ONCE - will be reused for all packets */
    aes128_ctr_init(&encrypt_state_c2s.ctx, encrypt_state_c2s.enc_key, encrypt_state_c2s.iv);
