...
main() {
    ...
    log "  Port:     $PORT"
    log "  Username: $VALID_USERNAME"
    log "  Password: $VALID_PASSWORD"
    log ""
    log "Connect with SSH client:"
    log "  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -p $PORT $VALID_USERNAME@localhost"
    log ""
    log "Or automated (with sshpass):"
    log "  sshpass -p $VALID_PASSWORD ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -p $PORT $VALID_USERNAME@localhost"
    log ""
