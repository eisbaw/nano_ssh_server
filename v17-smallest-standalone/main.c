/*
 * Nano SSH Server - v17-smallest-standalone
 * Smallest fully standalone SSH server with zero external crypto dependencies
 * Based on v12-opt11 (most optimized) + v16-crypto-standalone (custom crypto)
 *
 * Features:
 * - Custom AES-128-CTR (no OpenSSL)
 * - Custom SHA-256 and HMAC-SHA256 (no libsodium)
 * - Custom DH Group14 key exchange (no libsodium)
 * - Custom RSA-2048 signatures (no libsodium)
 * - Custom bignum library (no external dependencies)
...
/*
 * RSA-2048 signing (for host key)
 *
 * Custom RSA implementation:
 * - rsa_generate_keypair(n, e, d) - generate 2048-bit RSA key pair
 * - rsa_sign(sig, msg, msglen, n, d) - sign message with private key
 */
                         ...
                         crypto_state_t *state, uint8_t *mac)
{
    ...
    /* Encrypt packet in-place using custom AES-128-CTR
     * The CTR counter automatically increments across calls */
    aes128_ctr_xor(&state->aes_ctx, packet, packet_len);
...
ssize_t recv_packet(int fd, uint8_t *payload, size_t payload_max) {
    ...
    if (encrypt_state_c2s.active) {
        ...
        /* Decrypt first block using custom AES-128-CTR
         * Counter automatically increments from previous packet */
        memcpy(decrypted_block, first_block, 16);
        aes128_ctr_xor(&encrypt_state_c2s.aes_ctx, decrypted_block, 16);
        ...
        if (remaining > 0) {
            ...
            /* Decrypt remaining bytes using custom AES-128-CTR
             * Counter continues automatically from first block */
            memcpy(packet_buf + 16, encrypted_remaining, remaining);
            aes128_ctr_xor(&encrypt_state_c2s.aes_ctx, packet_buf + 16, remaining);
