/*
 * Nano SSH Server - v14-opt12
 * World's smallest SSH server for microcontrollers
 *
 * Phase 3: Advanced string and memory optimization
 *
 * Optimizations applied:
 * - Shortened error messages to reduce .rodata section
 * - Error messages use abbreviated forms while maintaining clarity
 * - Focus on reducing binary size without impacting functionality
 */
...
void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    if (i == (int)sizeof(client_version) - 1 && client_version[i] != '\n') {
        send_disconnect(client_fd, SSH_DISCONNECT_PROTOCOL_ERROR, "ver too long");
        close(client_fd);
        return;
    }
    ...
    if (strncmp(client_version, "SSH-2.0-", 8) != 0) {
        send_disconnect(client_fd, SSH_DISCONNECT_PROTOCOL_VERSION_NOT_SUPPORTED, "SSH-2.0 only");
        close(client_fd);
        return;
    }
    ...
    if (client_kexinit[0] != SSH_MSG_KEXINIT) {
        send_disconnect(client_fd, SSH_DISCONNECT_PROTOCOL_ERROR, "need KEXINIT");
        close(client_fd);
        return;
    }
    ...
    if (kex_init_msg[0] != SSH_MSG_KEX_ECDH_INIT) {
        send_disconnect(client_fd, SSH_DISCONNECT_KEY_EXCHANGE_FAILED, "need KEX");
        close(client_fd);
        return;
    }
    ...
    if (service_request[0] != SSH_MSG_SERVICE_REQUEST) {
        send_disconnect(client_fd, SSH_DISCONNECT_PROTOCOL_ERROR, "need SVC_REQ");
        close(client_fd);
        return;
    }
    ...
    if (strcmp(service_name, "ssh-userauth") != 0) {
        send_disconnect(client_fd, SSH_DISCONNECT_SERVICE_NOT_AVAILABLE, "userauth only");
        close(client_fd);
        return;
    }
    ...
    if (strcmp(channel_type, "session") != 0) {
        ...
        failure_len += 4;
        failure_len += write_string(open_failure + failure_len, "bad chan", 8);
        failure_len += write_string(open_failure + failure_len, "", 0);  /* language tag */

        send_packet(client_fd, open_failure, failure_len);
        send_disconnect(client_fd, SSH_DISCONNECT_PROTOCOL_ERROR, "session only");
        close(client_fd);
        return;
    }
