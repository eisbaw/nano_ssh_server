...
/* Host key, generated once at startup (bss).  host_pt is the wire form of
 * the public point, 0x04 || x || y, which is also what the key blob and the
 * scalar multiplier's output want. */
static uint8_t host_sec[32];
static uint8_t host_pt[65];

/* KEXINIT name-lists and the algorithm names reused elsewhere: kex,
 * hostkey, cipher, compression.  "nistp256" is the tail of the hostkey
 * name; the mac and language lists are empty (an AEAD cipher carries its
 * own MAC, and OpenSSH skips MAC negotiation for it). */
static const char nl[] =
    "ecdh-sha2-nistp256\0" "ecdsa-sha2-nistp256\0"
    "chacha20-poly1305@openssh.com\0" "none";
#define NL_HOSTKEY (nl + 19)
#define NL_CURVE   (nl + 30)
...
static size_t build_kexinit(uint8_t *p) {
    ...
    static const uint8_t off[10] = { 0, 19, 39, 39, 73, 73, 69, 69, 73, 73 };
    size_t o = 0;
    p[o++] = MSG_KEXINIT;
...
/* A fresh P-256 scalar: 32 random bytes, clamped (see p256.h). */
static void rand_scalar(uint8_t *k) {
...
static void handle(void) {
    ...
    /* ECDH: ephemeral key pair, public point in wire form 0x04 || x || y */
    uint8_t epriv[32], epub[65], shared[64], H[32], z[32];
    rand_scalar(epriv);
    epub[0] = 4;
    p256_smult(epub + 1, epriv, p256_g);

    /* host key blob K_S */
    uint8_t ks[128]; size_t ksl = 0;
    ksl += put_str(ks, NL_HOSTKEY, 19);
    ksl += put_str(ks + ksl, NL_CURVE, 8);
    ksl += put_str(ks + ksl, host_pt, 65);
    ...
    if (GET32(kinit + 1) != 65 || kinit[5] != 4) return;
    p256_smult(shared, epriv, kinit + 6);   /* K = x coordinate */
    ...
    uint8_t k[32], rs[64];
    rand_scalar(k);
    ecdsa_sign(rs, host_sec, k, z);

    /* KEX_ECDH_REPLY: K_S, Q_S, signature blob = string name || string
     * (mpint r || mpint s); the inner string's length depends on the
     * leading bits of r and s, so the two lengths are filled in last. */
    uint8_t rep[512]; size_t rl = 0;
    rep[rl++] = MSG_KEX_ECDH_REPLY;
    rl += put_str(rep + rl, ks, ksl);
    rl += put_str(rep + rl, epub, 65);
    {
        uint8_t *sig = rep + rl;
        size_t sl = 4 + put_str(sig + 4, NL_HOSTKEY, 19);
...
int main(void) {
    sha_gentables();
    fp_one[FP_SIZE - 1] = 1;
    rand_scalar(host_sec);
    host_pt[0] = 4;
    p256_smult(host_pt + 1, host_sec, p256_g);
