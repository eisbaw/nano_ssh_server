...
/* RFC 7748 scalar clamping, applied in place: it is idempotent, and the
 * ephemeral key is used for nothing but these two multiplications, so a
 * scratch copy would only cost a memcpy. */
static void smult_u(uint8_t *out, const struct ed25519_pt *q,
		    uint8_t *private_key)
{
	struct ed25519_pt p;

	ed25519_prepare(private_key);
	ed25519_smult(&p, q, private_key);
	ed_to_u(out, &p);
}
