...
void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    if (send_data(client_fd, server_version_line, 21) < 0) goto cleanup;
    ...
    for (i = 0; i < (int)sizeof(client_version) - 1; i++) {
        ...
        if (n <= 0) goto cleanup; /* Network error */
        version_len++;
    ...
    if (strncmp(client_version, "SSH-2.0-", 8) != 0) goto cleanup;
    ...
    /* Send our KEXINIT */
    if (send_packet(client_fd, server_kexinit, server_kexinit_len) < 0) goto cleanup;
    
    /* Receive client's KEXINIT */
    client_kexinit_len_s = recv_packet(client_fd, client_kexinit, sizeof(client_kexinit));
    if (client_kexinit_len_s <= 0) goto cleanup; /* Network error */
    
    if (client_kexinit[0] != SSH_MSG_KEXINIT) goto cleanup;
    ...
    if (kex_init_len <= 0) goto cleanup;

    if (kex_init_msg[0] != SSH_MSG_KEX_ECDH_INIT) goto cleanup;

    /* Extract client ephemeral public key (Q_C) */
    if (read_string(kex_init_msg + 1, kex_init_len - 1,
                   (char *)client_ephemeral_public, 33, &client_eph_str_len) == 0) goto cleanup;

    if (client_eph_str_len != 32) goto cleanup;
    
    /* Compute shared secret K */
    if (compute_curve25519_shared(shared_secret, server_ephemeral_private,
                                  client_ephemeral_public) != 0) goto cleanup;
    ...
    if (send_packet(client_fd, kex_reply, kex_reply_len) < 0) goto cleanup;
    ...
    if (send_packet(client_fd, newkeys_msg, 1) < 0) goto cleanup;
    ...
    if (newkeys_len <= 0) goto cleanup;
    if (recv_newkeys[0] != SSH_MSG_NEWKEYS) goto cleanup;
