/*
 * Nano SSH Server - v12-dunkels1
 * World's smallest SSH server for microcontrollers
 *
 * Dunkels Optimization 1: Single Buffer + Zero-Copy
 *
 * This version applies Adam Dunkels' single buffer architecture:
 * - ONE global packet buffer for all operations (send/receive/crypto)
 * - Zero-copy: build packets directly in buffer, no memcpy
 * - In-place encryption/decryption
 * - Minimal buffer management code
 *
 * Inspired by uIP's single buffer architecture for embedded systems
 */
...
/* ======================
 * Single Buffer Architecture - Dunkels Optimization
 * ====================== */

/*
 * Single packet buffer for ALL operations - Dunkels optimization
 *
 * This buffer is used for:
 * - Building outgoing packets (send_packet)
 * - Receiving incoming packets (recv_packet)
 * - Encryption/decryption (in-place)
 * - Temporary packet assembly
 *
 * Only ONE operation uses this buffer at a time (single-threaded server).
 * This eliminates multiple buffer copies and reduces code size.
 */
 ...
 *
 * Dunkels optimization: Uses global packet_buf, eliminates local buffer
 */
int send_packet(int fd, const uint8_t *payload, size_t payload_len) {
    ...
    /* Build packet directly in global buffer */
    write_uint32_be(packet_buf, packet_len);
    packet_buf[4] = padding_len;
    memcpy(packet_buf + 5, payload, payload_len);

    /* Add random padding */
    randombytes_buf(packet_buf + 5 + payload_len, padding_len);
    ...
    if (encrypt_state_s2c.active) {
        ...
        if (aes_ctr_hmac_encrypt(packet_buf, total_len, &encrypt_state_s2c, mac) < 0) {
            return -1;
        }

        /* Send encrypted packet */
        if (send_data(fd, packet_buf, total_len) < 0) {
            return -1;
        }
 ...
 *
 * Dunkels optimization: Uses global packet_buf, no malloc, in-place decryption
 */
ssize_t recv_packet(int fd, uint8_t *payload, size_t payload_max) {
    ...
    if (encrypt_state_c2s.active) {
        ...
        /* Read first encrypted block (16 bytes) directly into packet_buf */
        n = recv_data(fd, packet_buf, 16);
        if (n <= 0) {
        ...
        /* Decrypt first block IN-PLACE to extract packet_length
         * OpenSSL EVP allows in-place decryption */
        if (!EVP_DecryptUpdate(encrypt_state_c2s.ctx, packet_buf, &len,
                               packet_buf, 16)) {
            return -1;
        }

        packet_len = read_uint32_be(packet_buf);
        ...
        /* Read remaining encrypted bytes directly into packet_buf */
        if (remaining > 0) {
            n = recv_data(fd, packet_buf + 16, remaining);
            if (n <= 0) {
                return n;
            }
        ...
        /* Copy payload (still need this copy as output goes to caller's buffer) */
        ...
        /* Read packet_length (4 bytes) directly into packet_buf */
        n = recv_data(fd, packet_buf, 4);
        if (n <= 0) {
            return n;  /* Error or connection closed */
        }
        if (n < 4) {
        ...
        /* Read rest of packet directly into packet_buf */
