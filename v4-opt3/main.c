...
ssize_t recv_packet(int fd, uint8_t *payload, size_t payload_max) {
    /* Static buffers to eliminate malloc/free overhead */
    static uint8_t packet_buf_static[MAX_PACKET_SIZE + 4];  /* Max packet + length field */
    static uint8_t encrypted_remaining_static[MAX_PACKET_SIZE];  /* For encrypted data */
    ...
    if (encrypt_state_c2s.active) {
        ...
        /* Use static buffer for full decrypted packet */
        packet_buf = packet_buf_static;
        ...
        if (remaining > 0) {
            encrypted_remaining = encrypted_remaining_static;
        ...
        /* Use static buffer for rest of packet */
        packet_buf = packet_buf_static;
