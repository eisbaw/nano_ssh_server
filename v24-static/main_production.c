 ...
 */
static void generate_curve25519_keypair(uint8_t *private_key, uint8_t *public_key) {
...
static int compute_curve25519_shared(uint8_t *shared, const uint8_t *private_key, const uint8_t *peer_public) {
 ...
 */
static inline size_t write_string(uint8_t *buf, const char *str, size_t str_len) __attribute__((always_inline));
 ...
 */
static size_t read_string(const uint8_t *buf, size_t buf_len, char *out, size_t out_max, uint32_t *str_len) {
    if (buf_len < 4) {
 ...
 */
static inline uint8_t calculate_padding(size_t payload_len, size_t block_size) __attribute__((always_inline));
 ...
 */
static void compute_hmac_sha256(uint8_t *mac, const uint8_t *key,
 ...
 */
static int aes_ctr_hmac_encrypt(uint8_t *packet, size_t packet_len,
 ...
 */
static int send_packet(int fd, const uint8_t *payload, size_t payload_len) {
 ...
 */
static ssize_t recv_packet(int fd, uint8_t *payload, size_t payload_max) {
    uint8_t header[4];
    uint8_t mac[32];  /* HMAC-SHA256 produces 32-byte MAC */
    uint32_t packet_len;
    uint8_t padding_len;
...
/*
 * Macro: Write name-list (SSH string format for algorithm lists)
 * Returns bytes written - converted to macro to eliminate function call overhead
 */
 ...
 */
static size_t build_kexinit(uint8_t *payload) {
    size_t offset = 0;
 ...
 */
static size_t write_mpint(uint8_t *buf, const uint8_t *data, size_t data_len) {
    size_t offset = 0;
    size_t i;
 ...
 */
static void compute_exchange_hash(uint8_t *hash_out,
 ...
 */
static void derive_key(uint8_t *key_out, size_t key_len,
 ...
 */
static int accept_client(int listen_fd, struct sockaddr_in *client_addr) {
...
/*
 * Helper: Send SSH_MSG_USERAUTH_FAILURE
...
static int send_userauth_failure(int fd) {
    ...
    msg[len++] = 0;  /* Partial success: FALSE */
    return send_packet(fd, msg, len);
}

/*
 * Helper: Send simple channel message (SUCCESS, FAILURE, EOF, CLOSE)
 * Returns 0 on success, -1 on error
 */
...
static void handle_client(int client_fd, struct sockaddr_in *client_addr,
    ...
    memcpy(server_version_line, "SSH-2.0-NanoSSH_0.1\r\n", 21);
    ...
    uint8_t recv_newkeys[4];  /* Reduced from 16: only need 1 byte + margin */
    ssize_t newkeys_len;
    ...
    if (strcmp(method, "password") == 0) {
        ...
        if (strcmp(username, VALID_USERNAME) == 0 &&
            ...
            /* Authentication failed - send USERAUTH_FAILURE */
            if (send_userauth_failure(client_fd) < 0) {
                close(client_fd);
                return;
            }
        }
    } else {
        /* Unsupported authentication method - send USERAUTH_FAILURE */
        if (send_userauth_failure(client_fd) < 0) {
            close(client_fd);
            return;
        }
    ...
    while (!requests_done) {
        ...
        if (strcmp(req_type, "pty-req") == 0) {
            ...
            /* Exec request - accept but ignore command */
            request_accepted = 1;
        ...
        if (want_reply) {
            uint8_t msg_type = request_accepted ? SSH_MSG_CHANNEL_SUCCESS : SSH_MSG_CHANNEL_FAILURE;
            if (send_channel_msg(client_fd, msg_type, client_channel_id) < 0) {
                close(client_fd);
                return;
            }
        }
    ...
    if (send_channel_msg(client_fd, SSH_MSG_CHANNEL_EOF, client_channel_id) < 0) {
        close(client_fd);
        return;
    }

    /* Send SSH_MSG_CHANNEL_CLOSE */
    if (send_channel_msg(client_fd, SSH_MSG_CHANNEL_CLOSE, client_channel_id) < 0) {
        close(client_fd);
        return;
    }
    
    /* Wait for client's SSH_MSG_CHANNEL_CLOSE */
    uint8_t client_close[8];  /* Reduced from 16: only need 5 bytes */
