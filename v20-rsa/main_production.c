/*
 * Nano SSH Server - v20-rsa (RSA/DH Implementation)
 * Using RSA instead of elliptic curve cryptography
 * - Custom AES-128-CTR implementation (no OpenSSL)
 * - Custom SHA-256 implementation
 * - Custom HMAC-SHA256 implementation
 * - RSA-2048 for host key signatures (instead of Ed25519)
 * - DH Group14 for key exchange (instead of Curve25519)
...
#include <arpa/inet.h>
#include "bigint.h"
#include "dh_group14.h"
#include "rsa_minimal.h"
...
/* Global RSA host key */
static rsa_key_t host_rsa_key;

/* Initialize RSA host key */
void init_rsa_host_key(void) {
    rsa_load_hardcoded_key(&host_rsa_key);
}
...
void handle_client(int client_fd, struct sockaddr_in *client_addr) {
    ...
    /* ======================
     * Phase 1.6: DH Group14 Key Exchange (RSA Host Key)
     * ====================== */

    dh_context_t dh_ctx;
    bigint_t client_dh_public;
    bigint_t shared_secret_bn;
    uint8_t server_dh_public[256];
    uint8_t client_dh_public_bytes[256];
    uint8_t shared_secret[256];
    uint8_t exchange_hash[32];
    uint8_t session_id[32];
    uint8_t host_key_blob[512];
    size_t host_key_blob_len;
    uint8_t signature[256];
    int sig_len;
    uint8_t kex_reply[4096];
    size_t kex_reply_len;
    uint8_t kex_init_msg[512];
    ssize_t kex_init_len;
    uint32_t client_dh_str_len;

    /* Initialize DH context and generate server ephemeral key pair */
    dh_init(&dh_ctx);
    dh_generate_keypair(&dh_ctx);
    dh_export_public(server_dh_public, 256, &dh_ctx);

    /* Build server host key blob (ssh-rsa format) */
    /* Format: string "ssh-rsa", mpint e, mpint n */
    host_key_blob_len = 0;
    host_key_blob_len += write_string(host_key_blob + host_key_blob_len, "ssh-rsa", 7);

    /* Public exponent (e = 65537 = 0x00010001) as mpint */
    uint8_t e_bytes[4] = {0x00, 0x01, 0x00, 0x01};
    /* Skip leading zero */
    host_key_blob_len += write_string(host_key_blob + host_key_blob_len,
                                      (const char *)(e_bytes + 1), 3);

    /* Modulus (n) as mpint */
    uint8_t n_bytes[256];
    bigint_to_bytes(n_bytes, 256, &host_rsa_key.n);
    host_key_blob_len += write_string(host_key_blob + host_key_blob_len,
                                      (const char *)n_bytes, 256);

    /* Receive SSH_MSG_KEXDH_INIT from client */
    ...
    if (kex_init_msg[0] != SSH_MSG_KEXDH_INIT) {
        send_disconnect(client_fd, SSH_DISCONNECT_KEY_EXCHANGE_FAILED,
                       "Expected KEXDH_INIT message");
        close(client_fd);
        return;
    }

    /* Extract client DH public key (e) */
    if (read_string(kex_init_msg + 1, kex_init_len - 1,
                   (char *)client_dh_public_bytes, 257, &client_dh_str_len) == 0) {
        close(client_fd);
        return;
    }

    if (client_dh_str_len > 256) {
        close(client_fd);
        return;
    }

    /* Import client DH public key */
    dh_import_peer_public(&client_dh_public, client_dh_public_bytes, client_dh_str_len);

    /* Compute shared secret K = client_public^server_private mod p */
    dh_compute_shared(&shared_secret_bn, &dh_ctx, &client_dh_public);
    bigint_to_bytes(shared_secret, 256, &shared_secret_bn);

    /* Compute exchange hash H */
    fprintf(stderr, "\n=== V20 EXCHANGE HASH INPUTS (RSA/DH) ===\n");
    fprintf(stderr, "RSA modulus (n) length: %d bytes\n", 256);
    ...
    compute_exchange_hash(exchange_hash,
                         ...
                         client_dh_public_bytes, client_dh_str_len,
                         server_dh_public, 256,
                         shared_secret, 256);
    ...
    /* Sign exchange hash with RSA host private key */
    if (rsa_sign(signature, &sig_len, exchange_hash, 32, &host_rsa_key) != 0) {
        close(client_fd);
        return;
    }

    fprintf(stderr, "RSA Signature (%d bytes): ", sig_len);
    for (int i = 0; i < sig_len && i < 32; i++) fprintf(stderr, "%02x", signature[i]);
    fprintf(stderr, "...\n=================================\n\n");

    /* Build SSH_MSG_KEXDH_REPLY */
    kex_reply_len = 0;
    kex_reply[kex_reply_len++] = SSH_MSG_KEXDH_REPLY;
    ...
    kex_reply_len += write_string(kex_reply + kex_reply_len,
                                  (const char *)server_dh_public, 256);
    ...
    /* Signature of H (ssh-rsa signature format) */
    {
        ...
        sig_blob_len += write_string(sig_blob + sig_blob_len, (const char *)signature, sig_len);
        kex_reply_len += write_string(kex_reply + kex_reply_len,
                                      (const char *)sig_blob, sig_blob_len);
    }

    /* Send SSH_MSG_KEXDH_REPLY */
    fprintf(stderr, "\n=== SENDING KEXDH_REPLY (RSA/DH) ===\n");
