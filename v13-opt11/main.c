/*
 * Nano SSH Server - v13-opt11
 * World's smallest SSH server for microcontrollers
 *
 * Phase 3: Advanced C language optimizations
 *
 * This version includes:
 * - Function attributes (hot/cold/const/pure)
 * - Branch prediction hints (likely/unlikely)
 * - Bit fields for flags
 * - Const correctness
 * - Register hints for hot loops
 */
...
ssize_t send_data(int fd, const void *buf, size_t len) __attribute__((hot));
ssize_t recv_data(int fd, void *buf, size_t len) __attribute__((hot));
 ...
 * Output: 32 bytes
 */
static inline void compute_sha256(uint8_t *out, const uint8_t *in, size_t inlen)
...
static inline void hash_init(hash_state_t *h) __attribute__((always_inline));
static inline void hash_init(hash_state_t *h) {
 ...
 *
 * Using bit fields to reduce structure size
 */
typedef struct {
    ...
    unsigned int active:1;/* 1 if encryption is active (bit field) */
} crypto_state_t;

static crypto_state_t encrypt_state_c2s = {0}; /* Client to server */
static crypto_state_t encrypt_state_s2c = {0}; /* Server to client */

static inline void hash_update(hash_state_t *h, const uint8_t *data, size_t len)
    __attribute__((always_inline));
static inline void hash_update(hash_state_t *h, const uint8_t *data, size_t len) {
    crypto_hash_sha256_update(&h->state, data, len);
}

static inline void hash_final(hash_state_t *h, uint8_t *out) __attribute__((always_inline));
static inline void hash_final(hash_state_t *h, uint8_t *out) {
 ...
 */
static inline void write_uint32_be(uint8_t *buf, const uint32_t value)
    __attribute__((always_inline));
static inline void write_uint32_be(uint8_t *buf, const uint32_t value) {
    buf[0] = (value >> 24) & 0xFF;
    buf[1] = (value >> 16) & 0xFF;
 ...
 */
static inline void write_uint64_be(uint8_t *buf, const uint64_t value)
    __attribute__((always_inline));
static inline void write_uint64_be(uint8_t *buf, const uint64_t value) {
    buf[0] = (value >> 56) & 0xFF;
    buf[1] = (value >> 48) & 0xFF;
 ...
 */
static inline uint8_t calculate_padding(const size_t payload_len, const size_t block_size)
    __attribute__((always_inline, const));
static inline uint8_t calculate_padding(const size_t payload_len, const size_t block_size) {
    const size_t total_len = 5 + payload_len;  /* 4 (packet_length) + 1 (padding_length) + payload */
    uint8_t padding = block_size - (total_len % block_size);

    if (unlikely(padding < MIN_PADDING)) {
        padding += block_size;
    }
 ...
 */
__attribute__((hot))
int send_packet(int fd, const uint8_t *payload, const size_t payload_len) {
    ...
    if (unlikely(payload_len > MAX_PACKET_SIZE - 256)) {
        return -1;
    }
 ...
 */
__attribute__((cold))
void send_disconnect(int fd, const uint32_t reason_code, const char *description) {
 ...
 */
__attribute__((hot))
ssize_t recv_packet(int fd, uint8_t *payload, const size_t payload_max) {
    uint8_t header[4];
    uint8_t mac[32];  /* HMAC-SHA256 produces 32-byte MAC */
    uint32_t packet_len;
    uint8_t padding_len;
    ...
    if (encrypt_state_c2s.active) {
        ...
        if (unlikely(packet_len < 5 || packet_len > MAX_PACKET_SIZE)) {
            return -1;
        }
 ...
 */
__attribute__((hot))
int accept_client(const int listen_fd, struct sockaddr_in *client_addr) {
 ...
 */
ssize_t send_data(const int fd, const void *buf, size_t len) {
    ssize_t sent = 0;
    ssize_t n;
 ...
 */
ssize_t recv_data(const int fd, void *buf, const size_t len) {
    ssize_t n;
 ...
 */
__attribute__((hot))
void handle_client(const int client_fd, struct sockaddr_in *client_addr,
    ...
    for (register int i = 0; i < (int)sizeof(client_version) - 1; i++) {
        ...
        /* Stop at \n */
        if (likely(client_version[i] == '\n')) {
            break;
        }
    }

    /* Check if we hit the buffer limit without finding newline */
    if (version_len >= (int)sizeof(client_version) - 1 && client_version[version_len - 1] != '\n') {
