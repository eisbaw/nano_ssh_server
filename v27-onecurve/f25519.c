 ...
 * storage.  This is the only long exponentiation chain in the binary:
 * both the inverse and the square root are expressed in terms of it.
 */
...
void f25519_inv__distinct(uint8_t *r, const uint8_t *x)
{
	...
	/* This is a prime field, so by Fermat's little theorem x^(p-1) = 1,
	 * and the multiplicative inverse is x^(p-2) = x^(2^255-21).
	 *
	 * Rather than spend a second 248-iteration chain on it, note that
	 *
	 *     8 * (2^252-3) + 3 = 2^255-24+3 = 2^255-21
	 *
	 * so  x^(p-2) = (x^((p-5)/8))^8 * x^3, i.e. exp2523() followed by
	 * three squarings and a multiply by x^3.  Six extra field multiplies
	 * buy back the whole duplicate exponentiation chain.
	 */
	exp2523(r, x, s);               /* r = x^(2^252-3)  */
	f25519_mul__distinct(s, r, r);  /* ^2               */
	f25519_mul__distinct(r, s, s);  /* ^4               */
	f25519_mul__distinct(s, r, r);  /* s = x^(2^255-24) */

	f25519_mul__distinct(r, x, x);  /* x^2              */
	f25519_mul__distinct(t, r, x);  /* t = x^3          */
	f25519_mul__distinct(r, s, t);
}
