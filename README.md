...
The smallest working build is **4,975 bytes** (`v29-p256`, fully static,
...
just build v29-p256             # recommended/smallest: 4.9 KB, fully static
...
|--------------|---------|--------|------------------------|--------------------------------------|
| v29-p256     |   4,975 | 4.9 KB | static, no libc        | Recommended/smallest: P-256 key exchange and host key on one modular multiplier |
...
Exact byte counts vary a little with the toolchain. `v29-p256`'s 4,975,
...
savings are 3,000 bytes (−37.6%), 1,971 bytes (−19.8%) and 2,128 bytes
...
x = sqrt(q/x)), every hash is taken in one shot over an assembled buffer, the
KEX reply is built in place and the ELF program header is packed into the
tail of the ELF header. The cost is speed: every field multiplication is
bit-serial, so a handshake takes ~0.6 s on a desktop against ~0.07 s for
...
- One curve, one multiplier, one hash (`v29-p256`): NIST P-256 for both the
  ...
  place; `-fno-pie` (the code had been position-independent for nothing) and
  the program header packed into the ELF header's unused fields
